...
theoretical = {
    '7b_7b': {'128': [1.2548, 1.1483, 1.0586, 1.2682], '256': [1.1169, 1.0792, 1.0334, 1.1335]},
    '13b_7b': {'64': [1.1955, 1.1012, 1.346531], '128': [1.1024, 1.0493, 1.1908]}
}
...
for row, ac in enumerate(actor_critic):
    ...
    for b in bsz:
        tag = f"{ac}_{b}"
        axs[row][bsz.index(b)].bar(index, data[tag], bar_width, alpha=opacity, color=colors[1], label='PUZZLE', yerr=data_error[tag], capsize=5)
        axs[row][bsz.index(b)].bar(index, [(th-d) for th, d in zip(theoretical[ac][b], data[tag])], bar_width, bottom=data[tag], alpha=opacity, color=colors[0], label='Theoretical', capsize=5)
        axs[row][bsz.index(b)].text(0.9, 1.2, f'batch size {b}', fontsize=12, color='black', ha='center')
        if bsz.index(b) == 0:
            axs[row][bsz.index(b)].set_ylabel('Speedup')
        axs[row][bsz.index(b)].set_xticks([x + bar_width for x in index])
        axs[row][bsz.index(b)].set_xticklabels(config_label)
        axs[row][bsz.index(b)].set_ylim(0, 1.5)
        axs[row][bsz.index(b)].grid(axis='y', linestyle='-.')
        # axs[0][bsz.index(b)].set_title(f'Batch size {b}')
