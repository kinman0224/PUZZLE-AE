...
for loc, cluster in enumerate(clusters):
    ...
    axs.text(39, 0.5, f'on {cluster}', fontsize=12, color='black', ha='center')

    left = np.zeros(len(target_labels))
    for i, (s, ts) in enumerate(data.items()):
        bars = axs.barh(index, ts, bar_width, label=stages_label[i], left=left, color=colors[i % len(colors)])
        left += np.array(ts)

        for bar, value in zip(bars, ts):
            text_x = bar.get_x() + bar.get_width() / 2
            text_y = bar.get_y() + bar.get_height() / 2
            axs.text(text_x, text_y, f'{value:.2f}', ha='center', va='center', color='black')

    axs.set_yticks(index)
    axs.set_yticklabels(target_labels)
    # set x range to 45
    axs.set_xlim(0, 45)
    ...
    axs.legend(loc='upper center', bbox_to_anchor=(0.5, 1.2), ncol=4)
