...
def main():
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, (batch_prompt, batch_unsupervised) in enumerate(
            ...
            torch.cuda.synchronize()
            ed = time.time()
            print_rank_0(f"Generation time: {trainer.generate_time}, exp gen time: {ed - st}", args.global_rank)
            trainer.generate_time = ed - st
            ...
            if exp_dataset is not None:
                ...
                print_rank_0(f"Training time: {training_time}")
            ...
            if args.early_finish > 0 and step == args.early_finish:
                print_rank_0("exit with early finished, for debug")
                break
