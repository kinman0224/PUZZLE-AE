...
def create_datasets(args, tokenizer, train_phase=3):
    ...
    if parallel_state.get_tensor_model_parallel_rank() == 0:
        ...
        if parallel_state.get_data_parallel_world_size() == 1:
            ...
            prompt_train_sampler = RandomSampler(prompt_train_dataset, generator=g)
        else:
        ...
        prompt_train_dataloader = DataLoader(
            ...
            batch_size=args.per_device_training_batch_size)
        num_update_steps_per_epoch = len(prompt_train_dataloader) * \
            (args.per_device_training_batch_size / args.per_device_generation_batch_size) * \
            args.ppo_epochs / args.gradient_accumulation_steps
        num_total_iters = int(args.num_train_epochs * num_update_steps_per_epoch)
        num_total_iters = torch.cuda.LongTensor([num_total_iters])
    else:
        num_total_iters = torch.cuda.LongTensor(1)
    # Broadcast num tokens.
    torch.distributed.broadcast(num_total_iters,
                                parallel_state.get_tensor_model_parallel_src_rank(),
                                group=parallel_state.get_tensor_model_parallel_group())

    return prompt_train_dataloader, num_total_iters[0].item()
...
def create_prompt_dataset(local_rank,
    ...
    # torch.distributed.all_reduce(buf_create_cache, group=parallel_state.get_data_parallel_group())
    ...
    if local_rank <= 0 and (buf_create_cache.item() != 0 or reload):
        ...
        if train_phase == 1 and sft_only_data_path:
            ...
            if sft_eval_datasets:  # Check if sft_eval_datasets is not empty
                ...
                eval_dataset = Subset(eval_dataset, shuffle_idx.tolist())
        print("Saving the dataset to the cache ...")
