...
def get_forward_backward_func(dual=False, bulk=False):
    ...
    if pipeline_model_parallel_size > 1:
        if parallel_state.get_virtual_pipeline_model_parallel_world_size() is not None:
            ...
            if dual:
                if bulk:
                    forward_backward_func = forward_backward_pipelining_without_interleaving_dual_bulk
                else:
                    forward_backward_func = forward_backward_pipelining_without_interleaving_dual
            else:
...
def forward_step(forward_step_func,
    ...
    if (parallel_state.is_pipeline_last_stage() if not reverse else parallel_state.is_pipeline_first_stage()):
...
def forward_backward_pipelining_without_interleaving_dual(*,
                                                     forward_step_func_1,
                                                     forward_step_func_2,
                                                     data_iterator_1: Union[Iterator, List[Iterator]],
                                                     data_iterator_2: Union[Iterator, List[Iterator]],
                                                     model_1: Union[torch.nn.Module, List[torch.nn.Module]],
                                                     model_2: Union[torch.nn.Module, List[torch.nn.Module]],
                                                     num_microbatches: int,
                                                     dtype_1: torch.dtype,
                                                     dtype_2: torch.dtype,
                                                     tensor_shape_1: Shape,
                                                     tensor_shape_2: Shape,
                                                     decoder_seq_length: Optional[int] = None,
                                                     grad_scaler: Callable = None,
                                                     sequence_parallel: bool = False,
                                                     forward_only: bool = False,
    ...
    if isinstance(model_1, list):
        assert len(model_1) == 1, \
            "non-interleaved pipeline parallelism does not support model chunking"
        model_1 = model_1[0]
    if isinstance(model_2, list):
        assert len(model_2) == 1, \
            "non-interleaved pipeline parallelism does not support model chunking"
        model_2 = model_2[0]
    if isinstance(data_iterator_1, list):
        assert len(data_iterator_1) == 1, \
            "non-pipeline-parallel schedule does not support model chunking"
        data_iterator_1 = data_iterator_1[0]
    if isinstance(data_iterator_2, list):
        assert len(data_iterator_2) == 1, \
            "non-pipeline-parallel schedule does not support model chunking"
        data_iterator_2 = data_iterator_2[0]

    # Disable async grad reductions
    if no_sync_func is None and isinstance(model_1, torchDDP):
        assert model_1.no_sync == model_2.no_sync
        no_sync_func = model_1.no_sync
    if no_sync_func is None:
        no_sync_func = contextlib.nullcontext
    no_sync_context = None
    def disable_grad_sync():
    ...
    model_1_type = get_model_type(model_1)
    model_2_type = get_model_type(model_2)

    rank = parallel_state.get_pipeline_model_parallel_rank()
    recv_1_tensor_shapes = get_tensor_shapes(rank=rank-1,
                                           model_type=model_1_type,
                                           tensor_shape=tensor_shape_1,
                                           decoder_seq_length=decoder_seq_length,
                                           sequence_parallel=sequence_parallel)
    send_1_tensor_shapes = get_tensor_shapes(rank=rank,
                                           model_type=model_1_type,
                                           tensor_shape=tensor_shape_1,
                                           decoder_seq_length=decoder_seq_length,
                                           sequence_parallel=sequence_parallel)
    recv_2_tensor_shapes = get_tensor_shapes(rank=rank-1,
                                           model_type=model_2_type,
                                           tensor_shape=tensor_shape_2,
                                           decoder_seq_length=decoder_seq_length,
                                           sequence_parallel=sequence_parallel)
    send_2_tensor_shapes = get_tensor_shapes(rank=rank,
                                           model_type=model_2_type,
                                           tensor_shape=tensor_shape_2,
                                           decoder_seq_length=decoder_seq_length,
                                           sequence_parallel=sequence_parallel)
    ...
    if not forward_only:
        input_tensors = []
        output_tensors = []
    forward_data_store_1 = []
    forward_data_store_2 = []

    microbatch_forward_i = 0
    microbatch_backward_i = 0
    pipeline_stages = parallel_state.get_pipeline_model_parallel_world_size()
    capacity_curr = pipeline_stages - parallel_state.get_pipeline_model_parallel_rank()

    is_first_time = True
    qwq = False
    while microbatch_forward_i != num_microbatches * 2 or microbatch_backward_i != num_microbatches * 2:
        # print(f"START rk={parallel_state.get_pipeline_model_parallel_rank()}, forward={microbatch_forward_i}, backward={microbatch_backward_i}")
        if not ((microbatch_forward_i // pipeline_stages) % 2):
            forward_recv_tensor_shapes = recv_1_tensor_shapes
            forward_send_tensor_shapes = send_1_tensor_shapes
            forward_model = model_1
            forward_model_type = model_1_type
            forward_data_store = forward_data_store_1
            forward_step_func = forward_step_func_1
            forward_dtype = dtype_1
            data_iterator = data_iterator_1
            # forward_num_microbatches_real = microbatch_forward_i / pipeline_stages / 2 * pipeline_stages + microbatch_backward_i % pipeline_stages
        else:
            forward_recv_tensor_shapes = recv_2_tensor_shapes
            forward_send_tensor_shapes = send_2_tensor_shapes
            forward_model = model_2
            forward_model_type = model_2_type
            forward_data_store = forward_data_store_2
            forward_step_func = forward_step_func_2
            forward_dtype = dtype_2
            data_iterator = data_iterator_2
            # forward_num_microbatches_real = microbatch_forward_i / pipeline_stages / 2 * pipeline_stages + microbatch_backward_i % pipeline_stages

        if not ((microbatch_backward_i // pipeline_stages) % 2):
            backward_recv_tensor_shapes = recv_1_tensor_shapes
            backward_send_tensor_shapes = send_1_tensor_shapes
            backward_model = model_1
            backward_model_type = model_1_type
            backward_dtype = dtype_1
        else:
            backward_recv_tensor_shapes = recv_2_tensor_shapes
            backward_send_tensor_shapes = send_2_tensor_shapes
            backward_model = model_2
            backward_model_type = model_2_type
            backward_dtype = dtype_2


        if not microbatch_backward_i and len(input_tensors) != capacity_curr - 1:
            # single forward
            # print(f"START7 rk={parallel_state.get_pipeline_model_parallel_rank()}, forward={microbatch_forward_i}, backward={microbatch_backward_i}")
            input_tensor = recv_forward(forward_recv_tensor_shapes, forward_dtype, timers=timers)
            output_tensor = forward_step(forward_step_func, data_iterator, forward_model, num_microbatches,
                                        input_tensor, forward_data_store,
                                        timers, collect_non_loss_data, forward_dtype, enable_autocast)
            send_forward(output_tensor, forward_send_tensor_shapes, timers=timers)
            ...
            microbatch_forward_i += 1
        elif microbatch_forward_i == num_microbatches * 2 and microbatch_backward_i != num_microbatches * 2:
            # single backward
            # print(f"START6 rk={parallel_state.get_pipeline_model_parallel_rank()}, forward={microbatch_forward_i}, backward={microbatch_backward_i}")
            if is_first_time:
                # if microbatch_backward_i == num_microbatches * 2 - 1:
                #     if grad_sync_func is None or rank == 0:
                #         enable_grad_sync()
                ...
                output_tensor_grad = recv_backward(backward_recv_tensor_shapes, backward_dtype, timers=timers)
                input_tensor_grad = \
                    backward_step(grad_scaler, input_tensor, output_tensor,
                                output_tensor_grad, backward_model_type, timers, deallocate_pipeline_outputs)
            else:
                is_first_time = True
            send_backward(input_tensor_grad, backward_send_tensor_shapes, timers=timers)
            microbatch_backward_i += 1
            # print(f"END6 rk={parallel_state.get_pipeline_model_parallel_rank()}, forward={microbatch_forward_i}, backward={microbatch_backward_i}")

        else:
            # print(f"START1 rk={parallel_state.get_pipeline_model_parallel_rank()}, forward={microbatch_forward_i}, backward={microbatch_backward_i}")
            if is_first_time:
                input_tensor = recv_forward(forward_recv_tensor_shapes, forward_dtype, timers=timers)
                is_first_time = False
            else:
                input_tensor = send_backward_recv_forward(
                    input_tensor_grad, forward_recv_tensor_shapes, forward_dtype, timers=timers)
            # backward forward
            output_tensor = forward_step(forward_step_func, data_iterator, forward_model, num_microbatches,
                                        input_tensor, forward_data_store,
                                        timers, collect_non_loss_data, forward_dtype, enable_autocast)
            output_tensor_grad = \
                send_forward_recv_backward(output_tensor,
                                           backward_recv_tensor_shapes, backward_dtype,
                                           timers=timers)
            ...
            if microbatch_forward_i != num_microbatches * 2:
                microbatch_backward_i += 1
    # exit()
    # Launch any remaining grad reductions
    if no_sync_context is not None:
        # enable_grad_sync()
        if grad_sync_func is not None:
            grad_sync_func(model_1.parameters())
            grad_sync_func(model_2.parameters())
    end_time = time.time()
    return forward_data_store_1, forward_data_store_2


def forward_backward_pipelining_without_interleaving_dual_bulk(*,
                                                     forward_step_func_1,
                                                     forward_step_func_2,
                                                     data_iterator_1: Union[Iterator, List[Iterator]],
                                                     data_iterator_2: Union[Iterator, List[Iterator]],
                                                     ...
                                                     optimizer_1,
                                                     optimizer_2,
                                                     opt_param_scheduler_1,
                                                     opt_param_scheduler_2,
                                                     num_microbatches: int,
                                                     dtype_1: torch.dtype,
                                                     dtype_2: torch.dtype,
                                                     tensor_shape_1: Shape,
    ...
    # Switch off inter-pipeline operations in both optimizers.
    # TODO: support the inter-pipeline operations, carrying information to the first stage
    optimizer_1.only_core = True
    optimizer_2.only_core = True
    ...
    microbatch_backward_i = 0
    pipeline_stages = parallel_state.get_pipeline_model_parallel_world_size()
    capacity_curr = min(pipeline_stages - parallel_state.get_pipeline_model_parallel_rank(), num_microbatches)

    is_first_time = True
    last_size_0 = 0
    last_size_1 = 1
    while microbatch_forward_i != num_microbatches * 2 or microbatch_backward_i != num_microbatches * 2:
        if not ((microbatch_forward_i // num_microbatches) % 2):
            forward_recv_tensor_shapes = recv_1_tensor_shapes
            forward_send_tensor_shapes = send_1_tensor_shapes
            forward_model = model_1
            forward_data_store = forward_data_store_1
            ...
            forward_step_func = forward_step_func_2
            forward_dtype = dtype_2
            data_iter = data_iter_2
            data_iterator = data_iterator_2

        if not ((microbatch_backward_i // num_microbatches) % 2):
            backward_recv_tensor_shapes = recv_1_tensor_shapes
            backward_send_tensor_shapes = send_1_tensor_shapes
            backward_model_type = model_1_type
            backward_dtype = dtype_1
        ...
        if not microbatch_backward_i and len(input_tensors) != capacity_curr - 1:
            # single forward
            input_tensor = recv_forward(forward_recv_tensor_shapes, forward_dtype, timers=timers)
            output_tensor = forward_step(forward_step_func, data_iterator[data_iter], forward_model, num_microbatches,
                                        input_tensor, forward_data_store,
                                        timers, collect_non_loss_data, forward_dtype, enable_autocast)
            send_forward(output_tensor, forward_send_tensor_shapes, timers=timers)
            ...
            send_backward(input_tensor_grad, backward_send_tensor_shapes, timers=timers)
            microbatch_backward_i += 1
            if (microbatch_backward_i % (num_microbatches * 2) == num_microbatches + 1)\
                and microbatch_backward_i != last_size_0:
                last_size_0 = microbatch_backward_i
                if args.empty_unused_memory_level >= 1:
                    torch.cuda.empty_cache()
                enable_grad_sync()

                optimizer_1.reduce_model_grads(args, timers)
                update_successful, _, _ = optimizer_1.step(args, timers)

                # Gather params.
                if update_successful:
                    optimizer_1.gather_model_params(args, timers)
                    increment = num_microbatches * \
                ...
                if args.DDP_impl == 'local' and args.use_contiguous_buffers_in_local_ddp:
                    model_1.zero_grad_buffer()
                optimizer_1.zero_grad()

            if microbatch_backward_i % (num_microbatches * 2) == 1 and microbatch_backward_i != last_size_1:
                last_size_1 = microbatch_backward_i
                if args.empty_unused_memory_level >= 1:
                    torch.cuda.empty_cache()
                enable_grad_sync()
                optimizer_2.reduce_model_grads(args, timers)
                update_successful, _, _ = optimizer_2.step(args, timers)
            ...
            if microbatch_backward_i % (num_microbatches * 2) == 1 and microbatch_backward_i != last_size_1:
                ...
                if args.DDP_impl == 'local' and args.use_contiguous_buffers_in_local_ddp:
                    model_2.zero_grad_buffer()
                optimizer_2.zero_grad()
            data_iter = data_iter_2 if (microbatch_forward_i // num_microbatches) % 2 else data_iter_1
            # backward forward
            output_tensor = forward_step(forward_step_func, data_iterator[data_iter], forward_model, num_microbatches,
                                        input_tensor, forward_data_store,
                                        timers, collect_non_loss_data, forward_dtype, enable_autocast)
            ...
            deallocate_pipeline_outputs = None
    ...
    optimizer_1.only_core = False
    optimizer_2.only_core = False
